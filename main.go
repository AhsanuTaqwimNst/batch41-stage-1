...
func main() {
	...
	route.HandleFunc("/send-data-add-project", middleware.UploadFile(sendDataAddProject)).Methods("POST")
	route.HandleFunc("/form-edit-project/{id}", formEditProject).Methods("GET")
	route.HandleFunc("/send-data-edit-project/{id}", middleware.UploadFile(sendDataEditProject)).Methods("POST")
...
func home(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		if Data.IsLogin != true {
			...
			var result []projectDataStruc
			data, _ := connection.Conn.Query(context.Background(), "SELECT id, project_name, start_date, end_date, duration, description, technologies, image FROM db_project WHERE db_project.author_id=$1 ORDER BY id DESC", Data.UserId)
...
func sendDataAddProject(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		_, err = connection.Conn.Exec(context.Background(), "INSERT INTO db_project(author_id, project_name, start_date, end_date, duration, description, technologies, image) VALUES ($1, $2, $3, $4, $5, $6, $7, $8)", Data.UserId, projectName, startDate, endDate, duration, description, technologies, image)
		if err != nil {
			w.WriteHeader(http.StatusInternalServerError)
			w.Write([]byte("message : " + err.Error()))
			return
...
func login(w http.ResponseWriter, r *http.Request) {
	...
	err = bcrypt.CompareHashAndPassword([]byte(user.Password), []byte(password)) //perbandingan
	if err != nil {
		store := sessions.NewCookieStore([]byte("SESSION_KEY"))
		session, _ := store.Get(r, "SESSION_KEY")
